...
% \title{Hunyuan3D 2.5 Technical Report}
% \title{Hunyuan3D 2.5: High-Fidelity and Detailed 3D Assets Generation via Training and Test-Time Scaling}
\title{\shortname: Generation of 3D Assets}

% 参考：https://arxiv.org/pdf/2401.17807
...
% % review 3d generation: optimization based, feedforward (gan/vae/diffusion/ar/lrm), nvs
...
% % the importance of the dataset in the 3d field, why we re-org the dataset
...
% 3. 介绍最近的一些3D数据集
% 4. 介绍数据处理对训练模型的重要性
% 5. 简单介绍我们开源数据的数量、特别的地方(512 分辨率的水密)
...
% Representative works include Hunyuan3D 2.1, which leverages the rich and diverse data from Objaverse to achieve remarkable performance in generating high-fidelity and diverse 3D objects, demonstrating the transformative impact of large-scale complex datasets on 3D generation research.
...
\section{Methods}
% intro our method
% \subsection{Optimization-Based Generation}

% \subsection{Feedforward Generation}
...
% \subsection{Generative Novel View Synthesis}
% xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
% We combine manual annotation and automated conversion workflows to uniformly convert all 3D assets into single-frame static mesh representations with a Z-up coordinate system.
...
% % \textbf{Watertight Processing.} Give a artist-created triangle mesh, we first compute the Unsigned  Distance Field on a 512-resolution uniform grid, and extract the epsilon-contour thin mesh $\mathcal{M}$ using marching cube (epsilon=1/512). We further sample points on $\mathcal{M}$ and apply a delaunay triangulation to build the tetrahedra volume. We optimize the tetrahedra cell label as 0(inner)/1(outer) based on graphcut following ConvexMeshing[], and extract the boundary surface as the final watertight mesh.

% \textbf{Watertight Processing.} Given an artist-created triangle mesh, we first compute the Unsigned Distance Field (UDF) on a uniform grid with $512^3$ resolution, and extract an $\epsilon$-contour thin shell mesh $\mathcal{M}$ using Marching Cubes with $\epsilon=1/512$. We then sample points on $\mathcal{M}$ and apply Delaunay triangulation to construct a volumetric tetrahedral mesh. Following the approach of ConvexMeshing~\cite{}, we optimize the tetrahedral cell labels (0 for inner, 1 for outer) using graph cut optimization, and extract the boundary surface as the final watertight mesh.

% % \textbf{Point Sampling.} Following the sampling strategies of Dora and Hunyuan3D 2.1, we implement a hybrid sampling scheme on watertight meshes to ensure that the sampled point clouds can both adequately represent the overall geometric shape and accurately capture local detail features.
% % Specifically, the sampling strategy primarily consists of two complementary sampling steps:
% % (1) Surface Uniform Sampling. We first perform uniform sampling on the surface of watertight meshes to obtain a complete representation of the overall geometric shape. This sampling method ensures that point cloud density is independent of local geometric complexity, providing the model with a stable, globally consistent geometric representation.
% % (2) Edge Importance Sampling. Relying solely on uniform sampling often fails to adequately represent the geometric details of objects, particularly at locations with salient features such as edges, corners, and high-curvature regions. Therefore, we further introduce an edge importance sampling strategy to perform denser sampling in these geometrically significant regions. This adaptive sampling strategy can significantly enhance the point cloud's ability to express geometric details, enabling the model to better learn and reconstruct complex local structures.

% \textbf{Point Sampling.} Following the sampling strategies of Dora and Hunyuan3D 2.1, we implement a hybrid sampling scheme on watertight meshes by combining surface uniform sampling and edge importance sampling to ensure that the sampled point clouds can both adequately represent the overall geometric shape and accurately capture local detail features.
...
% objaverse:
% 3d assets (diverse format) -> blender render -> ply format -> watertight -> geometry verify ( geo quality, thin obj, overlap faces)

% generated data:
% text2text: categories to descriptions -> text2image: descriptions to images -> image23d: images to 3d assets -> geometry verify
...
% Additionally, considering that many products come with packaging in real-world scenarios (particularly in retail, logistics, and other application scenarios), we generate corresponding packaging descriptions for each item. This step significantly enhances the dataset's practicality, enabling it to better support downstream applications that need to consider packaged objects, such as warehouse robot operations and product recognition tasks.
...
% In the actual generation process, we adopt a hierarchical generation strategy: when generating images of objects themselves, we directly use the object description as the prompt; when generating packaged objects, we combine the packaging description with the object description. This strategy ensures that the generated packaged object images are both realistic and credible while maintaining the necessary associations.
...
% \subsection{Data Distribution, Composition and Visualization}

\subsection{Data Distribution and Visualization}

% {\bf Full-level Data}. To 
...
\begin{figure}[h]
    \centering
    % \includegraphics[width=0.9\linewidth]{figures/full_data_categories.png}
...
% The open-source data release provides a complete asset package, specifically including: (1) Processed watertight meshes: PLY format mesh files that have undergone rigorous topology repair and quality verification, topologically closed and free of self-intersections, directly usable for SDF-based implicit representation learning; (2) Sampled point clouds: High-quality point cloud data generated by combining surface uniform sampling and edge importance sampling strategies, containing surface points and normal vector information; (3) Rendered images: Multi-view, multi-projection mode (orthographic/perspective) rendered images, including RGB texture maps and surface normal maps. All data has undergone complete preprocessing and quality verification, ensuring it is fully ready-to-use. Researchers do not need to perform additional data cleaning or format conversion and can directly use it for training and evaluating 3D generation models.

% {\bf Part-level Data}. The part-level data contains \textcolor{red}{xxx} samples, with \textcolor{red}{xx}\% samples contain 2-5 components, \textcolor{red}{xx}\% samples contain 5-10 components, \textcolor{red}{xx}\% samples contain 10-20 components and the rest contain 20-50 components. The overall distributions are illustaed in Fig.
...
% {\bf Generated Data}. The Generated Data contains a total of 255,200 samples, with half being standalone product assets and the other half being packaged product assets. The category system design of this dataset fully considers the needs of real-world applications, ultimately encompassing 20 top-level categories, 130 mid-level subcategories, and 1,252 fine-grained classifications of product data. The breadth and depth of this category system far exceed existing real datasets, with coverage ranging from daily necessities and consumer electronics to professional industrial products.

% % {\bf Generated Data}. To compensate for the shortcomings of real datasets in terms of category coverage and long-tail distribution, we construct a large-scale, high-quality generated dataset through the aforementioned AIGC-driven synthesis pipeline. The category system design of this dataset fully considers the needs of real-world applications, ultimately encompassing 20 top-level categories, 130 mid-level subcategories, and 1,252 fine-grained classifications of product data. The breadth and depth of this category system far exceed existing real datasets, with coverage ranging from daily necessities and consumer electronics to professional industrial products.

% % Considering the complexity and diversity of practical application scenarios, we generate two forms of 3D assets for each product category: (1) Standalone product assets: Clean 3D models containing only the product itself, suitable for application scenarios requiring clear geometric structures, such as 3D generation model training, geometric understanding, and shape analysis; (2) Packaged product assets: Combined 3D models containing products and their complete packaging, with packaging design maintaining visual and semantic consistency with the product content, suitable for application scenarios closer to the real world, such as retail scene simulation, warehouse robot operations, logistics automation, and product recognition tasks. This dual-form data design greatly enhances the application flexibility of the dataset. Researchers can choose to use standalone products or packaged products according to specific task requirements.
