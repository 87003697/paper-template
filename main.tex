...
\begin{abstract}
  Despite the progress of 3D native generators, the scarcity of diverse 3D data confines them to limited distributions, causing severe misalignment on complex and non-canonical structures. 
  To address this, we propose \textbf{On-Policy Rendering Editing Optimization (OREO)}, which reformulates the post-training of 3D native generators as an on-policy distillation process with dense supervision on rendered views, \textbf{leveraging external 2D priors to learn from in-the-wild images}. 
  At its core, the \textbf{Reinforced Editing Distillation (RED)} algorithm leverages a pre-trained 2D editing model with an improved FlowEdit strategy to generate geometrically consistent pseudo-ground truths on rendered views, which are then backpropagated via a differentiable renderer with a \textbf{contrastive objective} to ensure stability. 
  OREO constructs a differentiable rollout for Backpropagation Through Time, supplemented by 3D Native Regularization to prevent geometric degradation. 
  Experiments show OREO outperforms supervised baselines on highly stylized and imaginative inputs, achieving superior conceptual consistency and detail restoration to mitigate the 3D data scarcity.
