...
In the third stage, OREO optimizes the 3D generator by backpropagating gradients from the final rendered image through the entire generation trajectory. 
Since the generation process is a discretized ODE solve (Eq. 165), the final state $z_0$ is a differentiable function of the initial noise $z_1$ and the model parameters $\theta$.
By applying the chain rule through the solver steps $z_{t_{i-1}} = \text{Solver}(z_{t_i}, v_\theta)$, we can compute the exact gradient $\nabla_\theta \mathcal{L}_{\text{RED}}$ via \textbf{Trajectory Backpropagation}.
This allows the 2D supervision to directly update the velocity field at every intermediate timestep, ensuring global consistency.
