...
\For{$i = 0$ to $N_e-1$} \Comment{\texttt{FlowEdit: Backward Euler Integration}}
    ...
    \State $\mathcal{L}_{src} \leftarrow \Call{ContrastiveLoss}{x_t^{src}, x^{src}, t}$ \Comment{\texttt{Source Loss}}
    \State $\mathcal{L}_{tgt} \leftarrow \Call{ContrastiveLoss}{x_t^{tgt}, x^{src}, t}$ \Comment{\texttt{Target Loss}}
...
\Procedure{ContrastiveLoss}{$x, y, t$}
    \State $x^{+} \leftarrow x - t \cdot v_\phi^{w}(x; t, c^{ref})$ \Comment{\texttt{Positive Anchor}}
    \State $x^{-} \leftarrow x - t \cdot v_\phi(x; t, \emptyset)$ \Comment{\texttt{Negative Anchor}}
    \State $w_{ada} \leftarrow (\| y - x^{+} \|_1 + \eta)^{-1}$ \Comment{\texttt{Adaptive Normalization}}
    \State \Return $w_{ada} \cdot (\| y - x^{+} \|^2 - \| y - x^{-} \|^2)$ \Comment{\texttt{Eq.~\ref{eq:contrastive_objective}}}
\EndProcedure
\end{algorithmic}
\end{algorithm}
