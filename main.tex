...
\begin{abstract}
  While 3D native generators have made significant strides, their generalization to highly stylized and non-canonical scenarios remains constrained by the scarcity of diverse 3D training data, often resulting in geometric distortions and conceptual misalignment.
  To address this challenge, we introduce \textbf{OREO}, a novel post-training paradigm that leverages the rich visual priors of pre-trained 2D models to compensate for 3D data paucity by establishing a ``Render-Edit-Distill-Optimize'' loop.
  At the core of our framework lies the \textbf{Reinforced Editing Distillation (RED)} algorithm. RED employs an enhanced FlowEdit strategy to inject target concepts into rendered views while preserving the original viewpoint, thereby generating geometrically consistent pseudo-ground truths. 
  These signals are subsequently distilled via a Contrastive Distillation Loss to ensure stable optimization.
  Built upon a Differentiable Rollout, OREO utilizes Backpropagation through the Trajectory to propagate terminal editing signals back to each generation step. 
  Furthermore, we incorporate a 3D Native Regularization to prevent geometric degradation from aggressive editing.
...
In recent years, 3D native generators such as Trellis and Hunyuan3D have demonstrated remarkable capabilities in automating high-quality 3D content creation. 
The current mainstream paradigm primarily relies on pre-training on large-scale 3D datasets. 
However, constrained by the prohibitive cost of 3D asset construction, existing 3D datasets lag far behind the 2D image domain in terms of diversity, which severely limits the generalization capability of models in non-canonical scenarios. 
This limitation is particularly critical in practical applications: when users attempt to create content that is imaginative, highly stylized, or even detached from realistic prototypes, pre-trained models often fail due to a lack of geometric priors, resulting in distorted structures or conceptual misalignment.

To address this challenge, our core insight is that while 3D data lacks diversity, 2D image editing models encapsulate rich general visual knowledge and can serve as ideal ``teachers''. 
By introducing these external priors into training, we can transfer the generalization capabilities of the 2D domain to 3D generation tasks. 
Based on this philosophy, we propose the \textbf{OREO (On-Policy Rendering Editing Optimization)} framework. 
This framework discards the traditional 3D supervised paradigm in favor of a ``Render-Edit-Distill-Optimize'' loop, reformulating post-training as an on-policy distillation process with dense supervision on rendered views. 
In this way, OREO can directly leverage feedback from the 2D teacher to optimize the 3D model, thereby eliminating the dependency on expensive 3D ground truth.
...
\begin{enumerate}
    ...
    Addressing the generalization bottleneck of pre-trained models in highly stylized and non-canonical scenarios, OREO effectively resolves the issue of insufficient training data diversity by constructing a ``Render-Edit-Distill-Optimize'' closed-loop mechanism.
